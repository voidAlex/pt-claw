...
# 死种通知状态（不存在则创建，schema 见 templates/pt_notify_state.example.json）
test -f pt_notify_state.json || echo '{"description":"通知状态追踪 — 控制死种提醒频率，防止重复通知刷屏","dead_torrents":{},"notify_config":{"dead_interval_hours":6,"dead_max_reminders":20}}' > pt_notify_state.json

# 关注列表（不存在则创建，schema 见 templates/pt_wishlist.example.json）
test -f pt_wishlist.json || echo '{"description":"关注列表 — 用户关注的影视、演员、番号，定时追剧用","movies":[],"actors":[],"fanhao":[]}' > pt_wishlist.json
```
...
| CookieCloud定时同步 **或** PT站点Cookie保活 | 每 4 小时 / 每天 06:00 | **二选一**（视 CookieCloud 配置），互斥不同时创建 |
