...
- **页面**: `https://www.pttime.org/adults.php?search=...&search_area=1&incldead=1`（注意参数是 `search=`，不是旧版 `searchstr=`）
- **按番号**: `search=SSIS-448&search_area=1&incldead=1`（走 adults.php，早期 `searchstr=` 参数曾失效但 `search=` 正常）
