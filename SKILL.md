...
> ⚠️ **`--tracker` 参数注意**：`qb_monitor.py --tracker <file>` 的 `_read_tracker()` 期望 tracker 文件是单个 epoch 时间戳整数，不是 hash 列表。如果文件包含 hash（如 cron 的 `pt_completed_last.txt`），会解析失败退回 epoch 0，展示全部完成记录。Cron 进度检查应使用 hash-based 方式（见 [references/cron-progress-check-pattern.md](references/cron-progress-check-pattern.md)），不用 `--tracker`。
...
Cron job 执行「PT下载进度检查」时，不要用 `qb_monitor.py --tracker`（epoch 格式，与 hash 文件不兼容）。正确流程：

1. `python3 scripts/qb_monitor.py --full` 获取全量数据
2. 读 `pt_completed_last.txt` 构建 known_hashes 集合
3. 筛选 progress==1.0 且 hash 不在集合中的 → 新完成
4. 筛选 progress==0 且 stalledDL 且 added_on 超过 7 天 → 死种
5. 有事件 → 报告并追加新 hash 到 tracker；无事件 → `[SILENT]`

详细实现见 [references/cron-progress-check-pattern.md](references/cron-progress-check-pattern.md)。
...
- **Method**: 仅 POST
- **Proxy**: API 从国内直连 403，需走 `PT_PROXY`。`mteam_api.py` 自动读取 `PT_PROXY`
...
**6. M-Team API**：①限速 403（1000次/24h）②下线 405 ③DNS 302 ④**国内 IP 直连 403，需走 PT_PROXY**。②③跳过馒头。`code` 是字符串 `"0"`。`genDlToken` 限频返空 bytes。`mteam_api.py` 自动走 `PT_PROXY`。
...
**28. pt_completed_last.txt 格式：hash 列表，不是 epoch**：Cron 进度检查使用 hash-based 追踪（见 `references/cron-progress-check-pattern.md`）。文件每行一个 torrent hash（40 字符 hex）。`qb_monitor.py --tracker` 的 `_read_tracker()` 期望 epoch 整数——两者不兼容，混用会导致 tracker 退回 epoch 0，每次 cron 都展示全部完成记录。Cron 不要用 `--tracker`，应手动做 hash 集合对比。
