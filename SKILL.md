...
# 全站（需代理的站用 with_proxy.sh 包装，绝不设全局 HTTP_PROXY）
...
### qb_add.py — 添加到 qBittorrent（含站点标签 + 正片提取）

```bash
# 公开磁链推荐加 --video-only，只下正片跳过广告垃圾文件
python3 scripts/qb_add.py "magnet:?xt=urn:btih:ABCDEF..." --tags sukebei --video-only

# PT 种子不需要过滤（PT 站一般只有正片）
python3 scripts/qb_add.py "https://pt.example.com/download.php?id=123" --category "电影" --tags pttime

# stdin JSON 模式（支持 video_only 字段）
echo '{"magnet": "...", "category": "<分类名>", "save_path": "<路径>", "tags": ["sukebei"], "video_only": true}' | python3 scripts/qb_add.py --stdin
```

`--video-only` 逻辑：添加种子暂停 → 等元数据 → 三级策略选正片：
...
**当用户第一次使用 PT 下载功能时（之前没有相关 memory），必须主动询问以下信息，并用 `memory()` 记录：**
...
7. **通知偏好**：下载完成后通知到哪里？
8. **代理配置**：代理地址、哪些站需要代理。
9. **Jellyfin 集成（可选）**：是否有 Jellyfin 服务器？几个？分别什么用途（影视/成人/音乐）？地址和 API Key？→ 没配置也能正常下载，只是无法自动去重。**API key 获取后必须同时写入 `~/.hermes/.env` 和 memory**，否则跨会话丢失。
10. **javbus-api（可选）**：是否需要部署 javbus-api（`docker run -d ovnrain/javbus-api`）获取封面预览和结构化磁链？→ 不部署也能裸爬 JavBus，只是磁链无结构化数据、无封面预览。
11. **PT 刷流（可选）**：是否需要自动刷流保号？→ 需要指定站点、免费种子条件、qBittorrent 专用目录、做种天数上限、并发数上限。
12. **成人内容下载偏好（可选）**：PT 做种阈值（低于N则走公开源）、版本优选顺序（做种数/去码/字幕/画质）、清晰度范围（4K/1080p/720p）。

所有回答用 `memory(action='add', target='memory')` 记录，避免下次再问。
...
```bash
# 下载历史（不存在则创建空文件）
test -f ~/.hermes/pt_downloaded.json || echo '{"description":"下载历史","items":[]}' > ~/.hermes/pt_downloaded.json

# 完成通知跟踪（不存在则创建）
test -f ~/.hermes/pt_completed_last.txt || echo "0" > ~/.hermes/pt_completed_last.txt

# 关注列表（不存在则创建空模板）
test -f ~/.hermes/pt_wishlist.json || echo '{"movies":[],"actors":[],"fanhao":[]}' > ~/.hermes/pt_wishlist.json
```
...
1. 有下载完成的 → 通知「✅ xxx 下载完成」，更新 ~/.hermes/pt_completed_last.txt
...
1. 读 ~/.hermes/pt_wishlist.json + pt_downloaded.json
...
# 仅直连不通时用 with_proxy.sh 包装
bash scripts/with_proxy.sh python3 scripts/pt_search.py "关键词" --limit 10
```

**成人区搜索** — 仅 PTTime 和 M-Team 有成人区，不走全站搜索：
- **PTTime 成人区**：页面 `adults.php`，搜索参数 `searchstr`（不是 `search`！）
  ```bash
  bash scripts/with_proxy.sh python3 scripts/pt_search.py "SONE-833" --site pttime --adult --limit 10
  ```
- **M-Team 成人区**：API `/torrent/search` 可能需加 `mode` 参数（API 限速 1000次/24h，超限返回 403）
  ```bash
  bash scripts/with_proxy.sh python3 scripts/pt_search.py "SONE-833" --site mteam --adult --limit 10
  ```

### Step 3：展示结果 & 排序

从用户 memory 读取偏好（清晰度、编码），按以下优先级排序：
...
**⚠️ 成人内容下载规则**（具体阈值和优先级从用户 memory 读取，以下为默认示例）：

- **PT 站阈值**：从 memory 读取（如「做种 ≥ N 才从 PT 下载」），低于阈值触发公开源回退
- **优选顺序**：从 memory 读取（如「做种数 > 去码 > 字幕 > 画质」）
- **去码关键词**：`uncensored` `破解` `無碼` `無修正` `Reducing Mosaic` `破坏版` `破壊版` `RM` `leak` `无码`
- **字幕关键词**：`中文字幕` `中文` `字幕` `-C` `-ch` `subtitle`
- **清晰度偏好**：从 memory 读取（如「4K/1080p 都接受」）
...
**前置验证 — 确保 API key 确实在 `.env` 中（不在仅存 memory）**：

Memory 中有 API key ≠ `.env` 中有 API key。跨会话后 memory 可能还在但 key 未落地到 `.env`，导致 401。每次做去重前先跑：

```bash
bash scripts/env_check.sh
```

如果 Jellyfin key 缺失，**直接用 `clarify()` 向用户索要**（不要猜测、不要跳过、不要尝试其他来源），拿到后写入 `~/.hermes/.env` 再继续。

**🛡️ 下载历史防重复（最高优先级，先于 Jellyfin 检查！）**

⚠️ **致命场景**：用户下载了某部片 → 看了不喜欢 → 手动从 Jellyfin/qB 删除 → 定时任务发现「片库缺失」→ 又给下回来了。**下载历史记录就是防这个的。**

每条推送成功的下载，必须记录到 `~/.hermes/pt_downloaded.json`：
...
**路径选择逻辑**：
- 成人/番号内容：检查用户 memory 是否有默认路径 → 有则直接用，无则列出 qBittorrent 分类让用户选
- 影视内容：根据类型匹配对应分类 → 用户 memory 有默认路径则直接用
- 其他内容：列出分类让用户选
...
**🎬 公开磁链正片提取（`--video-only`）**：Sukebei、JavBus 等公开源的种子常附带广告图片、URL 文件、垃圾 txt。推送时必须加 `--video-only`（或 stdin JSON 中 `"video_only": true`），只下载最大视频文件 + 匹配的字幕，其余跳过。PT 站种子不需要此参数（PT 资源干净）。
...
> ⚠️ **skill 中不硬编码分类名和路径**。用户的分类信息存于 memory 和 `.env`，Agent 运行时从 API 动态读取后匹配。
...
用户在 Step 0 被询问刷流偏好，生成 `~/.hermes/pt_boost.json`：
...
| 字段 | 说明 |
|------|------|
| `sites` | 刷流站点列表（目前支持 mteam） |
| `boost_category` | qBittorrent 分类名，专用于刷流 |
| `boost_save_path` | 刷流专用下载目录（独立于正常下载） |
| `max_seed_days` | 做种多少天后自动删除 |
| `max_torrents` | 同时做种上限 |
| `freeleech_only` | 是否仅下载免费种子 |
| `min_size_gb` / `max_size_gb` | 种子大小范围 |
| `min_seeders` | 最少做种人数 |
...
2. **站点是否需代理**：直接 curl 首页，连接超时/被墙 → 用 `bash scripts/with_proxy.sh curl ...` 包装
3. **是否有成人区**：问用户「这站有成人区吗？」→ 有则 `new_site.has_adult = true`
4. **认证方式**：
   - 优先问用户有没有 API Key（站内 控制面板 → API Token）
   - 没有 API → 用 Cookie。让用户从浏览器导出或提供 Cookie 字符串

Cookie 存入 `~/.hermes/.env`：`PT_COOKIE_<SITE>`（站点名大写，如 `PT_COOKIE_HDTIME`）
...
```bash
# 抓搜索页并保存
bash scripts/with_proxy.sh curl -b "c_secure_uid=...; c_secure_pass=..." \
...
```bash
# 1. 验证搜索返回结果
bash scripts/with_proxy.sh python3 scripts/pt_search.py "test" --site <new_site> --limit 3

# 2. 验证下载链接可访问
bash scripts/with_proxy.sh curl -I -b "<cookie>" "<download_url>" | head -3
...
3. **写入 memory**：站点名、代理需求、成人区有无、解析器类型
4. **告知用户**：适配完成，附验证结果
...
**认证方式**：Header `X-MediaBrowser-Token: <api_key>`（存用户 memory）
...
**配置**：Jellyfin 地址和 API key 必须同时存入 `~/.hermes/.env`（如 `JELLYFIN1_API_KEY=xxx`、`JELLYFIN1_URL=<JF_ADULT_HOST>`）和 memory，skill 脚本从环境变量读取。仅存 memory 的 key 会在跨会话时丢失。

### 关注列表

存储文件：`~/.hermes/pt_wishlist.json`
...
cronjob(action='create',
    ...
    prompt="""自动追剧检查（只搜索展示，不自动下载）：
1. 读取 ~/.hermes/pt_wishlist.json 关注列表
2. **第一步：读 ~/.hermes/pt_downloaded.json 下载历史**，构建已下载集合
...
**1. 代理绝不能写全局 env**：`HTTP_PROXY` 设在 `.env` 会导致 Agent 自身 API 走代理，挂了直接失联。只按需 `-x`。
...
**10. API key 必须写 `.env`**：仅 memory 会丢失。用 `printf >>` 追加（`write_file` 替换敏感值）。
...
**14. javbus_magnet.py `dn` NameError（裸爬模式崩溃）**：`_is_spam(dn)` 调用在 `dn` 赋值之前。修复：将 `dn = urllib.parse.unquote(m.group(2) or "")` 移到 `if ih in seen` 块之后、`_is_spam` 之前。此 Bug 导致 `--scrape` 模式完全崩溃，公开磁链回退链路断裂。

**15. pt_ratio_boost.py 硬编码死种阈值覆盖配置**：`run` 模式中 `if age > 86400 * 2` 与 `max_seed_days` 配置不一致。修复：改为 `cfg["max_seed_days"] * 86400`。原代码在 2 天即 `delete_files=True` 删文件，与用户配置的 7 天保种期冲突。

**16. qb_backup.py 缺少 `re` 导入**：`_download_torrent()` 中调用 `re.match()` 但未 import re。修复：在 `import json, os, sys` 中加入 `re`。此 Bug 导致 `qb_public_cleanup.py`（调用 `backup_from_torrents`）在删除前备份阶段崩溃。
...
**18. 公开磁链**：JavBus > Sukebei（磁链多/去码/AI）、`--video-only`+传 code、下完删种保文件、卡死换不同 hash。
...
> ⚠️ **致命警告**：不要在 `.env` 中全局设置 `HTTP_PROXY` / `HTTPS_PROXY`！这会导致 Agent 自己的模型 API 调用也走代理，代理挂了就失联。

| 变量 | 说明 |
|------|------|
| `PT_PROXY` | PT 搜索代理地址（如 `<PROXY_HOST>`），`with_proxy.sh` 从 `.env` 读取 |

### 用户偏好（存入 memory，不做硬编码）

- 清晰度偏好（4K/1080p/720p）
- 编码偏好（HEVC/AVC/AV1）
- 下载路径分类映射
- 成人内容 PT 做种阈值 & 版本优选顺序
- 关注列表 (`~/.hermes/pt_wishlist.json`)
