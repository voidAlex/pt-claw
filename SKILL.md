...
**配置模板**：`templates/user-preferences.example.md` 包含所有配置维度和占位符。首次配置时读取此模板作为参考，逐项向用户确认，用真实值填充后写入本地 `user-preferences.md`（不入 Git）。敏感值（API key、密码、IP）写入 `~/.hermes/.env`。**不写入 memory**（memory 容量有限，偏好配置数据量大，全部走文件）。
...
7. **通知偏好**：下载完成后通知到哪里？
8. **代理配置**：代理地址、哪些站需要代理。
9. **Jellyfin 集成（可选）**：是否有 Jellyfin 服务器？几个？分别什么用途（影视/成人/音乐）？地址和 API Key？→ 没配置也能正常下载，只是无法自动去重。**API key 写入 `~/.hermes/.env`**，跨会话不丢失。
...
所有回答写入 `user-preferences.md`（本地文件，不入 Git）。敏感值同时写入 `~/.hermes/.env`。不使用 memory 存储偏好（容量有限）。
...
```bash
# 下载历史（不存在则创建空文件）
test -f ~/.hermes/pt_downloaded.json || echo '{"description":"下载历史 — 防止用户手动删除后定时任务重复下载","items":[]}' > ~/.hermes/pt_downloaded.json

# 完成通知跟踪（不存在则创建）
test -f ~/.hermes/pt_completed_last.txt || touch ~/.hermes/pt_completed_last.txt
...
4. **更新 `templates/config.env.example`** — 添加新站点的 Cookie/API key 环境变量
5. **告知用户**：适配完成，附验证结果
...
**配置**：Jellyfin 地址和 API key 必须存入 `~/.hermes/.env`（如 `JELLYFIN1_API_KEY=xxx`、`JELLYFIN1_URL=<JF_ADULT_HOST>`），skill 脚本从环境变量读取。偏好文件 `user-preferences.md` 记录实例数量和用途映射。
...
**14. ~~javbus_magnet.py `dn` NameError~~ [已修复]**：`dn` 赋值已移至 `_is_spam` 调用之前。

**15. ~~pt_ratio_boost.py 硬编码死种阈值~~ [已修复]**：已改用 `cfg["max_seed_days"] * 86400`。同时修复了 `MAX_DELETE_PER_RUN` 未定义和 dry-run `age_days` 变量错误。

**16. ~~qb_backup.py 缺少 `re` 导入~~ [已修复]**：`import` 行已包含 `re`。
